...
class BayesPred(Job):
    ...
    def run(self, key2corr, channel, outdir, **sample_kwargs):
        ...
        if os.path.exists(join(real_outdir, f'channel_{channel}')):
             ...
             df4bay_cor = pd.read_csv('/mnt/obob/staff/fschmidt/cardiac_1_f/data/cam_can_1_f_dataframe_1_145_irasa.csv')
             cur_df = zscore(df4bay_cor.query('Chan == %d' % channel).query(f'condition == "{key2corr}"')[['Slope', 'age']], axis=0)

             trace = bmb.Model('Slope ~ 1 + age', 
                              cur_df, 
                              dropna=True, 
